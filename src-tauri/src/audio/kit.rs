...
impl Kit {
    ...
    /// Load `<dir>/kit.json` and every sample it names.
    pub fn load(dir: &Path) -> Result<Kit, String> {
        ...
        let manifest: Manifest = serde_json::from_str(&text)
            .map_err(|e| format!("{} is not a kit manifest: {e}", manifest_path.display()))?;

        let mut pads = Vec::with_capacity(manifest.pads.len());
        for pad in manifest.pads {
            let path = dir.join(&pad.file);
            let bytes = std::fs::read(&path)
                .map_err(|e| format!("could not read {}: {e}", path.display()))?;
            let audio =
                decode_wav(&bytes).map_err(|e| format!("{} is not usable: {e}", path.display()))?;
        ...
        if pads.is_empty() {
            return Err(format!("{} has no pads", manifest_path.display()));
        }
        Ok(Kit {
            id: manifest.id,
            pads,
...
mod tests {
    ...
    fn the_shipped_kit_loads_with_every_pad_audible() {
        let kit = Kit::load(&kit_dir()).expect("the shipped kit must load");
    ...
    fn the_hats_choke_each_other_and_nothing_else_does() {
        ...
        let kit = Kit::load(&kit_dir()).unwrap();
